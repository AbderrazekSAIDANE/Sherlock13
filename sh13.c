   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				else if ((mx>=250) && (mx<300) && (my>=350) && (my<740))
				{
					...
					//printf("Le guiltGuess %d",guiltGuess[ind]);
				}
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					if (guiltSel!=-1)
					{
						printf("Le guiltGuess %d",guiltGuess[id]);
						sprintf(sendBuffer,"G %d %d",id, guiltSel);   ///////G
						sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
					}
					else if ((objetSel!=-1) && (joueurSel==-1))
					{
						sprintf(sendBuffer,"O %d %d",id, objetSel);	///////O
						sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
					}
					else if ((objetSel!=-1) && (joueurSel!=-1))
					{
						sprintf(sendBuffer,"S %d %d %d",id, joueurSel,objetSel);	///////S
						sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
					}
					goEnabled = 0;
				}
		...
		switch (gbuffer[0])
		{
			...
			// Message 'D' : le joueur recoit ses trois cartes et les symbôles
			case 'D':
				sscanf(gbuffer,"D %d %d %d %d %d %d %d %d %d %d %d",&b[0],&b[1],&b[2],&tableCartes[id][0],&tableCartes[id][1],&tableCartes[id][2],&tableCartes[id][3],&tableCartes[id][4],&tableCartes[id][5],&tableCartes[id][6],&tableCartes[id][7]);
			...
			case 'V':
				sscanf(gbuffer,"V %d %d %d %d %d %d %d %d",&id,&objetSel,&inter[0],&joueur[0],&inter[1],&joueur[1],&inter[2],&joueur[2]);
						tableCartes[joueur[0]][objetSel] = inter[0];
						tableCartes[joueur[1]][objetSel] = inter[1];
						tableCartes[joueur[2]][objetSel] = inter[2];
						printf("Le symbôles %d %d %d\n",inter[0],inter[1],inter[2]);
					
				break;
			case 'H':
				sscanf(gbuffer,"H %d %d %d",&tableCartes[joueurSel][objetSel],&joueurSel,&objetSel);
				case 'T':
					sscanf(gbuffer,"T %d %d %d",&gId,&jeu,&quit);
					if(jeu ==1)
						printf("La probabilité du joueur %s est faux",gNames[gId]);
					else 
						printf("Le joueur %s a gagné",gNames[gId]);
						break;
		}
		synchro=0;
        }
	...
	for (i=0;i<4;i++)
        	for (j=0;j<8;j++)
        	{
			if (tableCartes[i][j]!=-1) //pas de ce symbole si -1
			{
				char mess[10];
				if (tableCartes[i][j]==100)  //Pourquoi 100
					sprintf(mess,"*");
				else
